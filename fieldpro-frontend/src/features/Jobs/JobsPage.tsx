...
function JobsPage() {
  ...
  const handleBulkDeleteClick = async () => {
    ...
    if (
      !confirm(
        `Sei sicuro di voler eliminare ${selectedJobIds.length} job selezionati?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      console.error("Errore di rete bulk delete:", err);
        ...
        <div className="container d-flex justify-content-between align-items-center">
          <div>
            <h1 className="h3 mb-1">
              SiteOps Studio · FieldPro – Job Management
            </h1>
            <small>
              Soluzione demo per la gestione job e cantieri per tecnici sul
              campo
            </small>
          </div>
